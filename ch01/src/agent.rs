...
impl Agent {
    pub fn new(epsilon: f64, action_size: usize) -> Self {
        Self {
            epsilon,
            estimeta_q_s: vec![0.; action_size],
            n_s: vec![0; action_size],
            total_reward: 0.,
            reward_log: vec![],
            rates: vec![],
            step_count: 0,
    ...
    pub fn update(&mut self, action_id: usize, reward: f64) {
        self.n_s[action_id] += 1;
        self.estimeta_q_s[action_id] +=
            (reward - self.estimeta_q_s[action_id]) / self.n_s[action_id] as f64;
    }
